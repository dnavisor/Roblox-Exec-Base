...
#define checkType(L, narg, t) luaL_checktype(L, narg, t);
#define luaL_stackcheck(L, MIN, MAX, T) if (lua_gettop(L) < MIN) { T return 0; } else if (lua_gettop(L) > MAX) { lua_settop(L, MAX); } T